...
def create_app():
    ...
    if not os.path.exists(db_path):
        # Utilizamos open() para crear un archivo vacío
        with open(db_path, 'w'):
            pass
...
def create_app():
    ...
    app.config['SQLALCHEMY_TRACK_MODIFICATIONS'] = False
    app.config['SQLALCHEMY_DATABASE_URI'] = f'sqlite:///{db_path}'  # Ajustamos la URI de la base de datos
    db.init_app(app)

    # Configuración de rutas y recursos para Flask-RESTful
    api.add_resource(controllers.ClientesController, '/clientes')
    api.add_resource(controllers.ClienteController, '/cliente/<id>')
    api.add_resource(controllers.PartidosController, '/partidos')
    api.add_resource(controllers.PartidoController, '/partido/<id>')
